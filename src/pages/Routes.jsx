...
const NEAREST_AIRPORT = {
  ...
  // Major metros and rail/air hubs — listed so flight cards on city-to-city
  // routes (Bangalore → Mumbai etc.) name the actual airport and IATA code,
  // not just "Flight". The drive hint reflects distance to the city centre.
  bangalore:           { airport: 'Kempegowda (BLR)',              drive: '~1-1.5h from city centre' },
  bengaluru:           { airport: 'Kempegowda (BLR)',              drive: '~1-1.5h from city centre' },
  mumbai:              { airport: 'Chhatrapati Shivaji (BOM)',     drive: 'in city · ~30-60 min from south Mumbai' },
  bombay:              { airport: 'Chhatrapati Shivaji (BOM)',     drive: 'in city · ~30-60 min from south Mumbai' },
  delhi:               { airport: 'Indira Gandhi (DEL)',           drive: 'in city · ~30-60 min from centre' },
  'new delhi':         { airport: 'Indira Gandhi (DEL)',           drive: 'in city · ~30-60 min from centre' },
  chennai:             { airport: 'Chennai Intl (MAA)',            drive: 'in city · ~30-45 min from centre' },
  kolkata:             { airport: 'Netaji Subhas Chandra Bose (CCU)', drive: 'in city · ~45-60 min from centre' },
  calcutta:            { airport: 'Netaji Subhas Chandra Bose (CCU)', drive: 'in city · ~45-60 min from centre' },
  hyderabad:           { airport: 'Rajiv Gandhi Intl (HYD)',       drive: '~45 min – 1h from centre' },
  pune:                { airport: 'Pune (PNQ)',                    drive: 'in city · ~30 min from centre · or BOM ~3.5h drive' },
  ahmedabad:           { airport: 'Sardar Vallabhbhai Patel (AMD)', drive: 'in city · ~20-30 min from centre' },
  goa:                 { airport: 'Dabolim (GOI) — South · or Mopa (GOX) — North', drive: 'GOI ~1h to most beaches · GOX ~1h to North Goa' },
  panaji:              { airport: 'Dabolim (GOI) — South · or Mopa (GOX) — North', drive: 'GOI ~30 min · GOX ~1h' },
  panjim:              { airport: 'Dabolim (GOI) — South · or Mopa (GOX) — North', drive: 'GOI ~30 min · GOX ~1h' },
  kochi:               { airport: 'Cochin Intl (COK)',             drive: '~45 min – 1h from Ernakulam' },
  cochin:              { airport: 'Cochin Intl (COK)',             drive: '~45 min – 1h from Ernakulam' },
  trivandrum:          { airport: 'Trivandrum Intl (TRV)',         drive: 'in city · ~20 min from centre' },
  thiruvananthapuram:  { airport: 'Trivandrum Intl (TRV)',         drive: 'in city · ~20 min from centre' },
  jaipur:              { airport: 'Jaipur Intl (JAI)',             drive: 'in city · ~30 min from centre' },
  lucknow:             { airport: 'Chaudhary Charan Singh (LKO)',  drive: 'in city · ~30-45 min from centre' },
  agra:                { airport: 'Kheria (AGR)',                  drive: 'small airport · most fly into Delhi (DEL) ~3.5h drive' },
  varanasi:            { airport: 'Lal Bahadur Shastri (VNS)',     drive: '~45 min – 1h from ghats' },
  kashi:               { airport: 'Lal Bahadur Shastri (VNS)',     drive: '~45 min – 1h from ghats' },
  benares:             { airport: 'Lal Bahadur Shastri (VNS)',     drive: '~45 min – 1h from ghats' },
  guwahati:            { airport: 'Lokpriya Gopinath Bordoloi (GAU)', drive: 'in city · ~30 min from centre' },
  bhubaneswar:         { airport: 'Biju Patnaik (BBI)',            drive: 'in city · ~15 min from centre' },
  indore:              { airport: 'Devi Ahilyabai Holkar (IDR)',   drive: 'in city · ~30 min from centre' },
  nagpur:              { airport: 'Dr. Babasaheb Ambedkar (NAG)',  drive: 'in city · ~20-30 min from centre' },
  amritsar:            { airport: 'Sri Guru Ram Dass Jee (ATQ)',   drive: 'in city · ~30 min from centre' },
  coimbatore:          { airport: 'Coimbatore Intl (CJB)',         drive: 'in city · ~20 min from centre' },
  mangalore:           { airport: 'Mangaluru Intl (IXE)',          drive: 'in city · ~30 min from centre' },
  mangaluru:           { airport: 'Mangaluru Intl (IXE)',          drive: 'in city · ~30 min from centre' },
  udaipur:             { airport: 'Maharana Pratap (UDR)',         drive: 'in city · ~45 min from centre' },
  jodhpur:             { airport: 'Jodhpur (JDH)',                 drive: 'in city · ~20 min from centre' },
  patna:               { airport: 'Jay Prakash Narayan (PAT)',     drive: 'in city · ~20-30 min from centre' },
  visakhapatnam:       { airport: 'Visakhapatnam Intl (VTZ)',      drive: 'in city · ~30 min from centre' },
  vizag:               { airport: 'Visakhapatnam Intl (VTZ)',      drive: 'in city · ~30 min from centre' },
};
...
const NEAREST_STATION = {
  ...
  // Major metros and rail hubs — most have multiple stations; the entry
  // names the key options so train cards show actual station names
  // rather than nothing for city-to-city routes.
  bangalore:           { station: 'KSR Bengaluru (SBC) / Yeshwantpur (YPR) / Cantonment (BNC)', drive: 'in city — choose by route' },
  bengaluru:           { station: 'KSR Bengaluru (SBC) / Yeshwantpur (YPR) / Cantonment (BNC)', drive: 'in city — choose by route' },
  mumbai:              { station: 'CSMT / Mumbai Central (BCT) / Bandra Terminus / Lokmanya Tilak (LTT)', drive: 'in city — choose by route' },
  bombay:              { station: 'CSMT / Mumbai Central (BCT) / Bandra Terminus / Lokmanya Tilak (LTT)', drive: 'in city — choose by route' },
  delhi:               { station: 'New Delhi (NDLS) / Hazrat Nizamuddin (NZM) / Old Delhi (DLI) / Anand Vihar (ANVT)', drive: 'in city — choose by route' },
  'new delhi':         { station: 'New Delhi (NDLS) / Hazrat Nizamuddin (NZM) / Old Delhi (DLI) / Anand Vihar (ANVT)', drive: 'in city — choose by route' },
  chennai:             { station: 'Chennai Central (MAS) / Chennai Egmore (MS)', drive: 'in city — Central for north/west routes, Egmore for south' },
  kolkata:             { station: 'Howrah (HWH) / Sealdah (SDAH) / Kolkata (KOAA)', drive: 'in city — Howrah handles most long-distance' },
  calcutta:            { station: 'Howrah (HWH) / Sealdah (SDAH) / Kolkata (KOAA)', drive: 'in city — Howrah handles most long-distance' },
  hyderabad:           { station: 'Secunderabad (SC) / Hyderabad Deccan (HYB) / Kacheguda (KCG)', drive: 'in city — Secunderabad is the main hub' },
  pune:                { station: 'Pune Junction (PUNE)',     drive: 'in city · ~15 min from centre' },
  ahmedabad:           { station: 'Ahmedabad Junction (ADI)', drive: 'in city · ~10 min from centre' },
  goa:                 { station: 'Madgaon (MAO) — South · Vasco da Gama (VSG) · Thivim (THVM) — North', drive: 'MAO ~30 min to South Goa beaches · THVM ~45 min to North Goa' },
  panaji:              { station: 'Madgaon (MAO) / Thivim (THVM)', drive: 'MAO ~45 min · THVM ~30 min' },
  panjim:              { station: 'Madgaon (MAO) / Thivim (THVM)', drive: 'MAO ~45 min · THVM ~30 min' },
  kochi:               { station: 'Ernakulam Junction (ERS) / Ernakulam Town (ERN)', drive: 'in city · ERS handles most long-distance' },
  cochin:              { station: 'Ernakulam Junction (ERS) / Ernakulam Town (ERN)', drive: 'in city · ERS handles most long-distance' },
  trivandrum:          { station: 'Thiruvananthapuram Central (TVC)', drive: 'in city · ~10 min from centre' },
  thiruvananthapuram:  { station: 'Thiruvananthapuram Central (TVC)', drive: 'in city · ~10 min from centre' },
  jaipur:              { station: 'Jaipur Junction (JP)',     drive: 'in city · ~15 min from centre' },
  lucknow:             { station: 'Lucknow Charbagh (LKO) / Lucknow Junction (LJN)', drive: 'in city · Charbagh is the heritage main station' },
  agra:                { station: 'Agra Cantt (AGC) / Agra Fort (AF)', drive: 'in city · Cantt handles most long-distance' },
  varanasi:            { station: 'Varanasi Junction (BSB) / Banaras (BSBS) / Manduadih', drive: 'in city · BSB is the main hub near the ghats' },
  kashi:               { station: 'Varanasi Junction (BSB) / Banaras (BSBS)', drive: 'in city · BSB is the main hub' },
  benares:             { station: 'Varanasi Junction (BSB) / Banaras (BSBS)', drive: 'in city · BSB is the main hub' },
  guwahati:            { station: 'Guwahati (GHY)',           drive: 'in city · ~15 min from centre' },
  bhubaneswar:         { station: 'Bhubaneswar (BBS)',        drive: 'in city · ~10 min from centre' },
  indore:              { station: 'Indore Junction (INDB)',   drive: 'in city · ~15 min from centre' },
  nagpur:              { station: 'Nagpur Junction (NGP)',    drive: 'in city · ~10 min from centre' },
  amritsar:            { station: 'Amritsar Junction (ASR)',  drive: 'in city · ~10 min from centre' },
  coimbatore:          { station: 'Coimbatore Junction (CBE)', drive: 'in city · ~15 min from centre' },
  mangalore:           { station: 'Mangaluru Central (MAQ) / Mangaluru Junction (MAJN)', drive: 'in city · MAQ for Konkan Railway' },
  mangaluru:           { station: 'Mangaluru Central (MAQ) / Mangaluru Junction (MAJN)', drive: 'in city · MAQ for Konkan Railway' },
  udaipur:             { station: 'Udaipur City (UDZ)',       drive: 'in city · ~15 min from centre' },
  jodhpur:             { station: 'Jodhpur Junction (JU)',    drive: 'in city · ~10 min from centre' },
  patna:               { station: 'Patna Junction (PNBE)',    drive: 'in city · ~15 min from centre' },
  visakhapatnam:       { station: 'Visakhapatnam (VSKP)',     drive: 'in city · ~15 min from centre' },
  vizag:               { station: 'Visakhapatnam (VSKP)',     drive: 'in city · ~15 min from centre' },
};
